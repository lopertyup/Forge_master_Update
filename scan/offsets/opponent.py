"""
============================================================
  FORGE MASTER — Opponent capture offset table (scan/)
  ...
  Two layouts are supported:
    ...
    * ``data/opponent_offsets.json`` — overrides written by
      the calibration tool. When present, overrides this file
...
# ────────────────────────────────────────────────────────────
#
# scan/offsets/opponent.py is two parents below the project
# root (scan/ ← project root). The override JSON lives next to
# the JSON libraries so users only have one folder to back up.
...
# colour heuristics:
#   * a 1.2 % wide vertical strip on the right edge → rarity
#   * a 18 % square at the centre                  → age
...
_OVERRIDES_PATH = _DATA_DIR / "opponent_offsets.json"
...
def offsets_for_capture(width: int,
                        height: int,
