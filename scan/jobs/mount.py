...
def scan(
    ...
    *,
    ...
    try:
        raw = ocr_image(capture, debug_zone="mount")
        text = fix_ocr(raw, context="mount")
        slot = parse_companion_text(text)
    except Exception:
        log.exception("scan.jobs.mount: OCR parse failed")
    ...
    return ScanResult(
        ...
        debug={"slot_dict": {"Mount": slot}, "raw_text": raw, "ocr_text": text, "missing_fields": missing},
    )
