"""
============================================================
  FORGE MASTER — Generic text parsers

  Convert raw text copied from the game (or the result of an
  OCR pass) into stat dictionaries / metadata blobs. Pure
  string handling: no I/O, no persistence, no simulation.

  This module replaces the old ``backend/parser.py``. It is
  located under ``backend.scanner`` because parsing OCR text is
  conceptually part of the scanner pipeline — the result feeds
  ``ocr_parser`` and ``library_ops`` downstream.
============================================================
"""
